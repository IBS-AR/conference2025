...
sessions <- readRDS(here::here("book/data/sessions.rds")) |> 
  ...
  mutate(content = case_when(type %in% c("General", "Social") ~ titles,
                             TRUE ~ str_c("<b>", map_chr(titles, quarto::tbl_qmd_span), "</b> by <i>", speakers, "</i><details><summary>Abstract</summary>", map_chr(abstract, quarto::tbl_qmd_span), "</details>")))  |> 
  # mutate(content = case_when(type %in% c("General", "Social") ~ titles,
...
table_day_maker <- function(data, offset = 0, keynote = "0") {
  ...
  schedule |> 
    ...
    row_spec(which(schedule$start != ""), background = "#D8E1E9") |> 
