...
sessions <- readRDS(here::here("book/data/sessions.rds")) |> 
  ...
  mutate(content = case_when(type %in% c("General", "Social") ~ titles,
                             TRUE ~ str_c("\\textbf{", titles, "} by \\textit{", speakers, "}")))  |> 
  select(-location, -start_of_session, -num_of_talks, -abstract,
         -titles, -speakers, -time) 
table_day_maker <- function(data, offset = 0) {
  chair <- data |> 
    ...
    mutate(order = 0) |> 
    mutate(`Arc Cinema` = str_c("\\textbf{Session ", 1:n() + offset, "A} -- ", `Arc Cinema`),
           Theatrette = str_c("\\textbf{Session ", 1:n()  + offset, "B} -- ", Theatrette)) |> 
    mutate(`Arc Cinema` = str_replace(`Arc Cinema`, "Session 8A", "Invited Session: Statistics for Biosecurity Surveillance")) |> 
    ...
    mutate(`Arc Cinema` = str_replace(`Arc Cinema`, "Session 12A", "Invited Session: Methods and Practice in Agricultural Analytics")) |> 
    mutate(`Arc Cinema` = str_replace(`Arc Cinema`, "Session (1|6|9|14|16)A", "Keynote"))
