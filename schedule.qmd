...
.modal-section-name:first-letter {
...
/* Collection wrapper gets the background, radius, padding, and gap */
.profile-card-collection {
  display: flex;
  flex-direction: column; /* stack vertically */
  gap: 12px;              /* spacing between cards */
  background: #d6e5fba8;  /* moved from individual card */
  padding: 10px;           /* moved from individual card */
  border-radius: 8px;      /* moved from individual card */
  margin-bottom: 12px;     /* spacing to other objects */
}

/* For multiple talks */
.callout-title-container p{
  font-size: 1.1rem;
  font-weight: bold;
  color: black;
  margin-bottom: 0rem;
...
/* For multiple talks */
.callout-title-container .profile-card-collection {
  background: white;
  row-gap: 0.5rem;
  margin-bottom: 0rem;
  padding-top: 0rem;
...
/* Individual cards become simple flex containers */
.profile-card {
  display: flex;
  align-items: center;
  gap: 12px;              /* space between image and info */
...
/* For multiple talks */
.callout-title-container .profile-name {
  font-weight: 600;
}
...
/* For multiple talks */
.callout-title-container .profile-organization {
  font-style: italic;
  font-weight: 400;
}

/* For multiple talks */
div.toggle-talk.callout.callout-note {
  border-left-color: #aed9e0;
...
Click on a block to see the full speaker list, then click on the title of the talk to see the corresponding abstract.
...
div(class = "scroll-wrapper",
    ...
    legend(types = c("general", "social", "keynote", "workshop", "poster"),
           labels = c("General", "Social", "Talks", "Workshop", "Poster"))
    )
)
...
escape_js <- function(x) {
  if (is.null(x)) return("")
  x <- stringr::str_replace_all(x, "\\\\", "\\\\\\\\") # escape slash
...
<script>
    function openModal(eventId) {
      ...
      if (event.speakers) {
        ...
        speakerSection.className = 'modal-section';
    
        let html = `
          <div class="modal-section-name">Speakers</div>
          <div class="modal-section-content">
            <div class="profile-card-collection">`;
        ...
        modalBody.appendChild(speakerSection);
      }
    
      // ==== Generic fields ====
      const excludeKeys = new Set([
        'name', 'time', 'id',
        'type', 'detail_type', 'speakers', 'speaker_imgs', 'speaker_orgs',
        'par_id', 'par_total'
      ]);
    
      for (const key in event) {
        if (!event[key] || excludeKeys.has(key) || /^talk_\d/.test(key)) continue;
      ...
      // ==== Talks ====
      const talkKeys = Object.keys(event).filter(k => /^talk_\d+$/.test(k));
      if (talkKeys.length > 0) {
        // Wrap all talks in one section with title
        const talksSection = document.createElement('div');
        talksSection.className = 'modal-section';
    
        let talksHTML = `
          <div class="modal-section-name">${event["type"]}s</div>
          <div class="modal-section-content">`;
    
        for (const key of talkKeys) {
          const i = key.match(/^talk_(\d+)$/)[1];
          ...
          // Build speaker cards
          let cardsHTML = '<div class="profile-card-collection">';
          ...
          if (talkAbstract) {
            talksHTML += `
              <div class="toggle-talk callout callout-style-simple callout-note no-icon callout-titled" data-border-left="none">
            ...
            // If no abstract, show title + speakers only
            talksHTML += `
              <div class="talk-entry">
                <div class="talk-title"><p>${render(talkTitle[0])}</p></div>
                ${cardsHTML}
              </div>`;
          }
        }
    
        talksHTML += '</div>'; // close modal-section-content
        talksSection.innerHTML = talksHTML;
        modalBody.appendChild(talksSection);
      }
